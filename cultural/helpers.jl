...
function clustering_kmedoids()
    ...
    features = collect(Matrix(geo_data[:, 1:2])');
     # features to use for clustering
    result = kmedoids(dist, 15);
    M = get_clusters(result.assignments);
    return M
end

function clustering_fuzzy()
    f = open(joinpath(@__DIR__, "geodata.txt"), "r+");
    lines = readlines(f);
    geo_data = zeros(Float64,length(ESTACIONES),2);
    _C = zeros(Int64,length(CANDIDATAS));
    ...
    features = collect(Matrix(geo_data[:, 1:2])');
     # features to use for clustering
    result = fuzzy_cmeans(features,15,2);
    M = result.weights;
    return M
end
...
function init_solution()
    ...
    if instancia == 0 #SE REPITE EL MISMO CODIGO EN AMBOS CASOS
        while true
            _C,v_C = init_solution_C_fuzzy(); #solucion inicial con cluster generados
...
function grilla_db_kmeans()
    mem = Array{Float64}[];
    i = 0;
    mkpath("/home/guillermo/Desktop/Isaac/DB-grillas/k-means")
    ...
    while i < 30
        ...
        if flag == 0
            ...
            open(joinpath("/media/guillermo/Swap/ExperimentosIsaac/RVNS/Resultados finales/grillas", filename), "w") do file
...
function grilla_db_kmedoids()
    mem = Array{Float64}[];
    i = 0;
    mkpath("/media/guillermo/Swap/ExperimentosIsaac/RVNS/Resultados finales/grillas/kmedoids")

    while i < 30
        grilla = clustering_kmedoids();
        flag = 0;
        for j = 1:i
            if mem[j] == grilla
                flag = 1;
        ...
        if flag == 0
            ...
            open(joinpath("/media/guillermo/Swap/ExperimentosIsaac/RVNS/Resultados finales/grillas/kmedoids", filename), "w") do file
                writedlm(file, grilla);
            end
        end
    end
...
function grilla_db_fuzzy()
    mem = Array{Float64}[];
    i = 0;
    mkpath("/media/guillermo/Swap/ExperimentosIsaac/RVNS/Resultados finales/grillas/fuzzy")

    while i < 30
        grilla = clustering_fuzzy();
        flag = 0;
        for j = 1:i
            if mem[j] == grilla
                flag = 1;
        ...
        if flag == 0
            ...
            open(joinpath("/media/guillermo/Swap/ExperimentosIsaac/RVNS/Resultados finales/grillas/fuzzy", filename), "w") do file
                writedlm(file, grilla);
            end
        end
    end
...
function get_center_by_weight(zone)
    return sample(Array{Int}(1:452),Weights(weight[:,zone]));
end

function get_zone_by_weight(e,weight)
    return sample(Array{Int}(1:452),Weights(weight[e,:]));
end

function swap_center_weight_grid(C,Vec_C,k)
    _C = zeros(Int64,452);
    _C = copy(C);
    v_C = zeros(Int64,15);
    v_C = copy(Vec_C);
    ...
    for i=1:k
        ...
        #println("zona ",idx);
        #println("estacion que cambio ",v_C[idx]);
        #println("estacion nueva ",b[i]);
        #_C[a[i]] = 0;
        #_C[x] = 1;
        v_C[idx] = x;
    end
    _C = setindex!(zeros(Int64,452),ones(Int64,15),vec(v_C));
    return _C,v_C,a;
end

function init_solution_C_fuzzy()
    v_C = getindex.(findmax(weight,dims=1)[2],1)
    o_C = setindex!(zeros(Int64,452),ones(Int64,15),vec(v_C))
    return o_C,v_C
end
